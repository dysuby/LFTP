...
class Reciever:
    def __init__(self, addr, port, remotePath, localPath):
    ...
    def recv(self):
        ...
        kw = {Field.FILE_NAME: self.remotePath, Field.ACK: ACK,
              Field.EOF: self.done, Field.RWND: self.ws}
        while True:
            ...
            self.logger.log('receive SEQ: {} expected SEQ: {}'.format(rkw[Field.SEQ], ACK + 1))
    ...
    def handleData(self):
        ...
        while not self.f.closed:
            while len(self.buffer):
                ...
                debug += 1
                self.f.write(data)
                self.logger.log('Writing {} to file'.format(debug))
                if not data:
                    l.append(debug)
            if self.done:
                self.f.close()
    ...
    def putData(self, data):
        ...
        self.logger.log('len: {} {}'.format(len(data), data))
